...
class AnalyzerServicer(grpc_stub.AnalyzerServicer):
    ...
    def llmPrompt(self, request: analyzer_pb2.Prompt, context):
        start = time.time()
        print("H1")
        res = analys(services=["Docker", "PostgreSQL"], prompt_template=request.tmp, pmt=request.pmt)
        end = time.time()
        diff = end - start
        print("Time Taken: {} s".format(diff), res, end='\n')
        return analyzer_pb2.Prompt(pmt=res)
